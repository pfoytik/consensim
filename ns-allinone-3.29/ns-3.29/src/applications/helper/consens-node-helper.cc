...
#include "ns3/consens-node-helper.h"
...
ConsensHelper::ConsensHelper (std::string protocol, Address address, std::vector<Ipv4Address> peers, int noMiners,
...
{
  m_factory.SetTypeId ("ns3::Consens");
  ...
  ConsensHelper::InstallPriv (Ptr<Node> node) //FIX ME
{
   switch (m_minerType)
   {
      case NORMAL_MINER:
      {
        Ptr<Consens> app = m_factory.Create<Consens> ();
...
enum MinerType
ConsensHelper::GetMinerType(void)
{
  return m_minerType;
}

void
ConsensHelper::SetMinerType (enum MinerType m)  //FIX ME
{
   m_minerType = m;
...
void
ConsensHelper::SetBlockBroadcastType (enum BlockBroadcastType m)
{
